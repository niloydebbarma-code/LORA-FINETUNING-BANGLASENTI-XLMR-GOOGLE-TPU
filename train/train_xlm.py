# Fine-Tuning for Bangla Sentiment Analysis on XLM-RoBERTa-Base using TPUs
...
def append_main_log(msg):
    ...
    main_log_file = os.path.join('logs', 'train_banglasenti_xlm.log')
...
def train_worker(flags):
    ...
    log_file = os.path.join(log_dir, 'train_banglasenti_xlm.log')
    ...
    logger.info("Model loaded and moved to XLA device.")
    ...
    best_acc = 0.0
    checkpoint_dir = os.path.join(config['train']['checkpoint_dir'], 'banglasenti-xlmr')
    ...
    try:
        ...
        for epoch in range(config['train']['epochs']):
            ...
            if acc > best_acc:
                ...
                best_weights_path = os.path.join(checkpoint_dir, 'xlmr_weights.pt')
                ...
                try:
                    ...
                    # Save tokenizer
                    tokenizer.save_pretrained(os.path.join(checkpoint_dir, 'xlmr_tokenizer'))
                    ...
                    logger.info(f"Model config and tokenizer saved in {checkpoint_dir}")
        ...
        final_weights_path = os.path.join(final_checkpoint_dir, 'final_xlmr_weights.pt')
        ...
        try:
            ...
            logger.info(f"Final model config and tokenizer saved in {final_checkpoint_dir}")
        except Exception as e:
...
if __name__ == '__main__':
    ...
    print("Starting fine-tuning for Bangla sentiment analysis")
