...
from models.lora_adapter import load_lora_adapters
...
def evaluate(model, dataloader, device):
    ...
    logging.info(f"Per-class support (true labels): {np.bincount(labels, minlength=model.config.num_labels)}")
    logging.info(f"Per-class predictions: {np.bincount(preds, minlength=model.config.num_labels) if len(preds) > 0 else []}")
    return acc, f1

def main():
    log_dir = 'logs'
    os.makedirs(log_dir, exist_ok=True)
    log_file = os.path.join(log_dir, 'eval_run.log')
    ...
    eval_config = {
        ...
        'adapter_dir_during': config['adapter_dir_during'],
        'adapter_weight_file_during': config['adapter_weight_file_during'],
        'tokenizer_path_during': config['tokenizer_path_during'],
        'base_model_path_final': config['base_model_path_final'],
        'adapter_dir_final': config['adapter_dir_final'],
        'adapter_weight_file_final': config['adapter_weight_file_final'],
    ...
    paths = {
        "during": {
            ...
            "adapter_dir": eval_config['adapter_dir_during'],
            "adapter_weight_file": eval_config['adapter_weight_file_during'],
            "tokenizer_path": eval_config['tokenizer_path_during']
        },
        "final": {
            "base_model_path": eval_config['base_model_path_final'],
            "adapter_dir": eval_config['adapter_dir_final'],
            "adapter_weight_file": eval_config['adapter_weight_file_final'],
    ...
    for model_type, model_paths in paths.items():
        ...
        adapter_dir = os.path.abspath(model_paths['adapter_dir'])
        ...
        # Load LoRA adapters from the specified directory
        adapter_config_path = os.path.join(adapter_dir, 'adapter_config.json')
        adapter_weights_path = eval_config[f'adapter_weight_file_{model_type}']
        model = load_lora_adapters(model, adapter_config_path, adapter_weights_path)
        model.to(device)
        ...
        logging.info(f"{model_type.capitalize()} model eval done. Accuracy={acc:.4f}, F1={f1:.4f}")
